...
public class AddDetailsController implements Initializable {
    ...
    private void addExperienceButtonClicked(MouseEvent event) throws NamingException {
        ...
        exp.setPosition(positionText.getText());
        exp.setOrginization(orginizationText.getText());
        ...
        exp.setEndDate(date2);
        Long expId = candidateProxy.createExperience(exp);
        candidateProxy.affecterExperienceCandidate(expId, Authenticator.currentUser.getId());
    ...
    private void addCursusButtonClicked(MouseEvent event) throws NamingException {
        ...
        cursus.setDegree(degreeText.getText());
        cursus.setUniversity(universityText.getText());
         Instant instant = Instant.from(cursusStartDate.getValue().atStartOfDay(ZoneId.systemDefault()));
        Date date = Date.from(instant);
        cursus.setStartDate(date);
        Instant instant2 = Instant.from(cursusEndDate.getValue().atStartOfDay(ZoneId.systemDefault()));
        Date date2 = Date.from(instant2);
        cursus.setEndDate(date2);
        Long cursusId = candidateProxy.createCursus(cursus);
        candidateProxy.affecterCursusCandidate(Authenticator.currentUser.getId(), cursusId);
